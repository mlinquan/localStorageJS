 ...
 */
 ...
 function localStorageJS(a, options, localStorageJSTag) {
    var qq = 0;
    var lsJ = lsCount = a.length, lsI=0, lsList = {}, lsOnload = [], lsPanding = {}, lsAllready = 0, second = new Date().getTime()/1000,
    ...
    release_panding = function(name) {
        ...
        if(lsCount == 0) {
            for(q in lsList) {
                ...
                if(lsList[q].type == 'js' && (lsList[q].removeSelf || config.removeAll) && lsList[q].lsStatus != "error") {
    ...
    for(var i=0;i<lsJ;i++) {
        ...
        if(!config.debug && localstorageable && localStorage.getItem(a[i].name)) {
            var scTmp = localStorage.getItem(a[i].name),
            thatTmp;
            try{
    ...
    window.onload = function() {
        if(!config.debug && localstorageable && lsOnload.length) {
            for(var g=0;g<lsOnload.length;g++) {
                var scTmp = localStorage.getItem(lsOnload[g].name),
                thatTmp;
                try{
                    thatTmp = JSON.parse(scTmp);
                } catch(e) {
